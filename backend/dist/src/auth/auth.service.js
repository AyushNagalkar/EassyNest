...
let AuthService = class AuthService {
    ...
    async register(dto) {
        if (dto.role === client_1.Role.ADMIN) {
            throw new common_1.ForbiddenException('Admin accounts cannot be self-registered');
        }
    ...
    async refresh(refreshToken) {
        try {
            ...
            if (this.redis) {
                const isRevoked = await this.redis.sismember(`revoked:${payload.sub}`, refreshToken);
                if (isRevoked) {
            ...
            if (this.redis) {
                ...
                await this.redis.expire(`revoked:${payload.sub}`, 7 * 24 * 60 * 60);
            }
