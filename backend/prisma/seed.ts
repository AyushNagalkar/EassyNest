import { PrismaClient, Role, RoomType, FurnishingStatus, ListingStatus, SeekerType, TargetType, InterestStatus, ScoreSource } from '@prisma/client';
...
async function main() {
  ...
  // 4. Create Seeker Profiles
  const profile1 = await prisma.seekerProfile.create({
    data: {
      userId: tenant1.id,
      type: SeekerType.ROOM_SEEKER,
  ...
  const profile2 = await prisma.seekerProfile.create({
    data: {
      userId: tenant2.id,
      type: SeekerType.FLATMATE_SEEKER,
      preferredCity: 'Bangalore',
  ...
  // 7. Seed Compatibility Scores (Pre-computed cache)
  // Seeker 1 (Amit) vs Prop 1 (Koramangala Cozy Single) -> 92
  ...
  // Seeker 1 (Amit) vs Prop 3 (Studio Layout Sector 1) -> 84
  await prisma.compatibilityScore.create({
    data: {
      seekerProfileId: profile1.id,
      targetType: TargetType.PROPERTY,
  ...
  // Seeker 3 (Vikram PM) vs Prop 2 (Spacious flat) -> 88
  await prisma.compatibilityScore.create({
    data: {
  ...
  // Seeker 2 (Priya UX) vs Seeker 3 (Vikram PM) -> 74 (Flatmate match)
  await prisma.compatibilityScore.create({
    data: {
      seekerProfileId: profile2.id,
      targetType: TargetType.SEEKER_PROFILE,
  ...
  // 8. Interests
  // Interest 1: Tenant 1 (Amit) -> Prop 1 (Ramesh) -> Accepted
  ...
  // Interest 2: Tenant 3 (Vikram) -> Prop 2 (Ramesh) -> Pending
  ...
  // Interest 3: Tenant 2 (Priya) -> Seeker 3 (Vikram) -> Accepted (Flatmate)
  ...
  // Messages in Room 1 (Tenant 1 and Owner 1)
    ...
    data: [
      {
        ...
        createdAt: new Date(Date.now() - 3600000 * 24), // 24 hrs ago
      },
      {
        chatRoomId: room1.id,
        senderId: owner1.id,
  ...
  // Messages in Room 2 (Tenant 2 and Tenant 3)
  await prisma.message.createMany({
    data: [
      {
        chatRoomId: room2.id,
