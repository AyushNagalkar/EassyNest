          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              <div className="flex lg:hidden border border-[var(--border)] rounded-[var(--radius)]">
                <button onClick={() => setViewMode('list')} className={`p-2 ${viewMode === 'list' ? 'bg-[var(--accent-flatmate-light)] text-[var(--accent-flatmate)]' : 'text-[var(--foreground-muted)]'}`}>
                  <List className="h-4 w-4" />
                </button>
                <button onClick={() => setViewMode('map')} className={`p-2 ${viewMode === 'map' ? 'bg-[var(--accent-flatmate-light)] text-[var(--accent-flatmate)]' : 'text-[var(--foreground-muted)]'}`}>
          ...
          {filtersOpen && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} className="overflow-hidden">
              <div className="flex gap-3 pt-4">
                ...
                <Button variant="ghost" size="sm" onClick={() => setCity('')}><X className="h-3 w-3" /> Clear</Button>
              </div>
            </motion.div>
          )}
        </div>
        ...
        <div className={`flex-1 overflow-y-auto lg:max-w-[60%] ${viewMode === 'map' ? 'hidden lg:block' : ''}`}>
          <div className="mx-auto max-w-3xl px-4 py-6">
            {loading ? (
              ...
              <EmptyState icon={<Users className="h-8 w-8" />} title="No flatmate seekers found" description="Try searching a different city." />
            ) : (
              <>
                        ...
                        <div className="card card-hover p-5">
                          <div className="flex items-start gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              <h3 className="font-semibold text-[var(--foreground)] group-hover:text-[var(--accent-flatmate)] transition-colors truncate">
                                {seeker.user?.name}
                              </h3>
                          ...
                          {seeker.bio && (
                            <p className="text-xs text-[var(--foreground-secondary)] mt-3 line-clamp-2">{seeker.bio}</p>
                          )}
                ...
                {meta.totalPages > 1 && (
                  <div className="flex items-center justify-center gap-2 mt-8">
                    ...
                    <span className="text-sm text-[var(--foreground-secondary)]">Page {meta.page} of {meta.totalPages}</span>
